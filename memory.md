...
- **An empty composer is not proof a message was sent.** The send clicked the editable, inserted the text and submitted without checking the text had arrived. When focus does not land in the box the insert goes nowhere silently, and every remaining signal then agrees the send worked — Send is pressed on an empty box, the box is empty afterwards, and an empty box is exactly what a delivered message leaves behind. Targets came back as "the composer emptied but the message is not in the conversation", which describes a half-delivery, when nothing had been typed at all. Confirm the message *into* the composer before submitting (`_message_in_composer`), retry with the box cleared between attempts — a retry that appends to half-typed text sends both — and report "never reached the composer" as its own distinct state.

- **`inner_text` is always empty on a `<textarea>`.** It returns the markup's default text, not the live `value`. X's DM composer is a textarea; Instagram's and TikTok's are contenteditable `<div>`s with no `value` at all. A composer check written with `inner_text` therefore answered "yes, it cleared" unconditionally on X — a no-op that guarded X sends for weeks. One reader (`_composer_text`) takes `value` where there is one and `innerText` otherwise.

- **TikTok's `message-input-area` is a wrapper, not the input.** It contains the editable *and* the "Send a message..." placeholder, so typing into it went to a container and reading it back always found text. An emptied composer read as non-empty on every send and 24 delivered messages were recorded as failures. Target `[data-e2e='message-input-area'] div[contenteditable='true']`; keep the wrapper only as a last resort. The general lesson: "is the box empty" and "has the message gone" are the same question only when the selector resolves to the editable itself, so test the *message*, not the emptiness.

- **X's profile header renders long after `domcontentloaded`.** A flat 2.2s pause read `state=None` on roughly half of profiles — including four the account was already following, reported as "no follow button". The follow buttons appear at ~3s, sometimes later. Poll for the control (6 × 1.5s) instead of guessing; a fixed pause tuned on a fast profile silently mislabels the slow ones, and "no follow button" is indistinguishable from a genuine block in the log.

- **TikTok's web client does not show the whole inbox.** Message requests to people who do not follow back never render at `/messages` — this account had 10 conversations on web against 25 confirmed sends. There is no Requests folder in the web UI. Any "who actually received it" audit has to come off the phone; the web sidebar will quietly under-report and look authoritative doing it. The sidebar also lists **display names, not handles** (`radszzz.rev` shows as "radzxx🪖"), so matching inbox rows to campaign targets by handle silently misses people.

- **A stale `postmaster.pid` survives a reboot and blocks Postgres for ever.** The file records the old postmaster's PID; after a reboot that number gets recycled to an unrelated process (here, macOS's QuickLook thumbnail agent). Postgres sees a live process at the recorded PID, concludes another postmaster owns the data directory, and refuses to start — every 10s under `KeepAlive`, all night. The backend then dies at startup on `ConnectionRefusedError`, which reads as a backend fault and is not one. Check `pgrep -f postgres` first: if it is empty the pid file is lying and can be moved aside. If it is *not* empty, do not touch the file — two postmasters on one data directory corrupt it. Note `brew services` is refused on this machine as an unsupported configuration; the launch agent at `~/Library/LaunchAgents/homebrew.mxcl.postgresql@16.plist` is what runs it.
